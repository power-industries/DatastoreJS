...
class Datastore {
	...
	read() {
		return new Promise((resolve, reject) => {
			fs.readFile(this.#filename, 'utf8', (readError, data) => {
				if(readError)
	...
	write() {
		return new Promise((resolve, reject) => {
			fs.mkdir(path.dirname(this.#filename), {recursive: true}, (mkdirError) => {
				if(mkdirError)
					return reject(mkdirError);

				try {
					fs.writeFile(this.#filename, JSON.stringify(this.#data), 'utf8', writeError => {
						if (writeError)
