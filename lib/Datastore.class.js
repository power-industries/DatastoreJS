...
class Datastore {
	...
	// Private Methods
	#serialize() {
		let serialData = JSON.stringify(this.#data.entries());
	...
	#deserialize(dataString) {
		...
		this.#data = new dMap(JSON.parse(dataString));
	}
	#generateUUID() {
		let id = uuid();
	...
	#checkIntegrity() {
		...
		this.#data.keys().forEach(key => {
			...
			if (!this.#config.schema.validateSync(data))
	...
	// Persistence
	readSync() {
		let data = fs.readFileSync(this.#config.filename, 'utf8');
	...
	insertMany(dataObjectArray) {
		...
		return dataObjectArray
			...
			.map(dataObject => {
				return cloneDeep(dataObject);
			})
			// Parse each dataObject
			.map(dataObject => {
				let result = this.#config.schema.parseSync(dataObject);

				if(!(result instanceof Type.Object))
	...
	updateOne(queryFunction, updateFunction) {
		...
		record.data = this.#config.schema.parseSync(updateFunction(record.data));
