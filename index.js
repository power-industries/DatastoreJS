...
class Datastore {
	 ...
	 * @param filename {String} - Path to the persistence File
	 */
	...
	/**
	 * Reads the Datastore from the persistence File asynchronously
	 * @returns {Promise<undefined>} - Returns a Promise which
	...
	write() {
		return new Promise((resolve, reject) => {
			try {
				fs.writeFile(this.#filename, this.#data, 'utf8', error => {
					if (error)
						return reject(error);
	 ...
	 * @param data {*}
	 * @param id {String}
	 * @return {Promise<Record>} - Resolves with the inserted Record on success
	 */
	insert(data, id = uuid()) {
		return new Promise((resolve, reject) => {
			if(typeof id !== 'string')
	...
	filter(f) {
		return new Promise((resolve, reject) => {
			...
			return resolve(Object.values(this.#data).filter(f));
		});
	}
